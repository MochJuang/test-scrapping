...
exports.execute = async function() {
    ...
    const url = 'https://www.lamudi.co.id/banten/tangerang-selatan/pamulang/buy/?q=pamulang+estate'
        ...
        .then(html => {
            ...
            $('.ListingCell-row').each( async(index, item) => {
                ...
                row.next().next().find('.RequestPhoneFormNumber').each( (i, v) => {
                    $(v).text().trim() ? agentContacts.push($(v).text().trim()) : false
                } )
                let details = []
                let facilities = []
            ...
            let data = JSON.stringify(listData, null, 4)
